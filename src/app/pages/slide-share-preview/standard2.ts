export interface RenderStandard2PreviewOptions {
...
export async function renderStandard2Preview(options: RenderStandard2PreviewOptions): Promise<string> {
  const {
    loadImage,
    bgUrl,
    useCorsForBg,
  ...
  ctx.drawImage(iconImg, horizontalPadding, mainHeight + verticalPadding + (usefulFooterHeight - iconH) / 2, iconW, iconH);
...
function drawWhiteBorderFooter(
  ...
  const { canvasWidth, mainHeight, footerHeight, horizontalPadding, verticalPadding, iconW, qrImg, currentFontFamilyName, smallText, bigText } = options;
  ...
  const qrX = canvasWidth - horizontalPadding - qrSize;
  ...
  const iconTextGap = footerHeight * 0.1;
  const textStartX = horizontalPadding + iconW + iconTextGap;
