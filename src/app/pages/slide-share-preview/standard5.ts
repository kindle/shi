export interface RenderStandard5PreviewOptions {
...
export async function renderStandard5Preview(options: RenderStandard5PreviewOptions): Promise<string> {
  const {
    loadImage,
    bgUrl,
    useCorsForBg,
  ...
  const bgScale = Math.max(leftImageWidth / bgImg.width, mainHeight / bgImg.height);
  const bgDrawWidth = bgImg.width * bgScale;
  const bgDrawHeight = bgImg.height * bgScale;
  const bgOffsetX = (leftImageWidth - bgDrawWidth) / 2;
  const bgOffsetY = (mainHeight - bgDrawHeight) / 2;

  ctx.save();
  ctx.beginPath();
  ctx.rect(0, 0, leftImageWidth, mainHeight);
  ctx.clip();
...
function drawRightSideVerticalStrip(
  ...
  const { rightStripX, rightStripWidth, mainHeight, iconImg, currentFontFamilyName, bigText, smallText, dateText } = options;
  ...
  const dateBottomY = mainHeight - stripPadding - dateFontSize;
