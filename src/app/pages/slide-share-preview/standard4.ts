export interface RenderStandard4PreviewOptions {
...
export async function renderStandard4Preview(options: RenderStandard4PreviewOptions): Promise<string> {
  const {
    loadImage,
    bgUrl,
    useCorsForBg,
...
function drawGrayscaleImageClipped(
  ctx: CanvasRenderingContext2D,
  options: {
  ...
  offscreen.width = Math.max(1, Math.round(imageBoxWidth));
  offscreen.height = Math.max(1, Math.round(imageBoxHeight));
  ...
  if (!offCtx) {
    ...
    ctx.clip();
    ctx.drawImage(sourceImage, bgOffsetX, bgOffsetY, bgDrawWidth, bgDrawHeight);
    ctx.restore();
    return;
  }

  // Draw with coordinates shifted into the clipped offscreen buffer.
  const shiftedX = bgOffsetX - imageBoxX;
  const shiftedY = bgOffsetY - imageBoxY;
  offCtx.drawImage(sourceImage, shiftedX, shiftedY, bgDrawWidth, bgDrawHeight);
  ...
  try {
    ...
    // On tainted canvas or restricted environments, gracefully keep the color image.
  }

  ctx.drawImage(offscreen, imageBoxX, imageBoxY, imageBoxWidth, imageBoxHeight);
}
